...
for user in usernames :
    if(user in str_input) : 
        style = f'<p style="font-family:Arial; background-color : rgb(255, 255 ,255 1); color : rgb(0,0,0,1)">{str_input[str_input.index(user)+1]}</p>'
        st.markdown(style, unsafe_allow_html=True)
    else :
        style = '<p style="font-family:Arial; background-color : rgb(255, 255 ,255 1); color : rgb(0,0,0,1); ">Nan</p>'
        st.markdown(style, unsafe_allow_html=True)
